...
- **Test iOS touch on overlays.** The site uses `scroll-snap-type: y mandatory` on `<html>`, so any modal/overlay must fully block touch panning or iOS Safari will scroll the page underneath (exposing the white body background below the last section). `touch-action` is **not inherited**: setting it on the overlay's root only covers fingers landing directly on the root. Any descendant with `pointer-events: auto` — the `<video>`/`<img>`, the scrub slider — keeps the default `touch-action: auto` and leaks a vertical drag to the browser, because the JS pointer handler only `preventDefault()`s horizontal swipes. Stamp `touch-action: none` onto every descendant (`.lightbox *`), not just the root. Verify by swiping vertically on every region of the overlay — caption pills, padding, peeks, and especially directly on the active media — none should scroll the page.
